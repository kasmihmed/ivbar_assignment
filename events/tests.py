...
class TestEvents(TestCase):
    ...
    def setUp(self):
        ...
        return CareGiver.objects.create(name='caregiver_name',
                                        location='caregiver_location')

    def test_create_events(self):
        client = APIClient()
        assert Event.objects.count() == 0
        response = client.post('/caregivers/1/events/',
    ...
    def test_filtering_by_month(self):
        ...
        first_event.time_stamp = first_event.time_stamp + \
        ...
        assert int(first_month) - int(second_month) == -1 or\
               int(first_month) - int(second_month) == 1
    ...
    def test_filtering_by_caregiver(self):
        client = APIClient()
        CareGiver.objects.create(name='caregiver_name 2',
                                 location='caregiver_location 2')
        response = client.post('/caregivers/1/events/',
                               {'event_type': 'event_type'},
                               format='json')
        assert response.status_code == 201
        response = client.post('/caregivers/2/events/',
                               {'event_type': 'event_type'},
                               format='json')
        assert response.status_code == 201
        assert Event.objects.count() == 2
